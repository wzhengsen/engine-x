...
def compile_lua(src, dst, lua_command):
    command = "%s -b \"%s\" \"%s\"" % (lua_command, src, dst)
    print("  Compiling %s " % src)
    ret = subprocess.call(command, shell=True, cwd = os.path.dirname(lua_command))
    if ret != 0 :
...
def sync_folder(src_dir, dst_dir, lua, need_compile):
    if os.path.isfile(src_dir):
        if not os.path.exists(os.path.dirname(dst_dir)):
            os.makedirs(os.path.dirname(dst_dir))
        if lua and need_compile:
            compile_if_newer(src_dir, dst_dir, lua)
        else:
        ...
        for name in names:
            ...
            sync_folder(src, dst, lua, need_compile)
    else:
...
if __name__ == "__main__":
    ...
    update_files = 0
    start_at = time.time()
    sync_folder(args.src_dir, args.dst_dir, args.lua, need_compile)
    end_at = time.time()
